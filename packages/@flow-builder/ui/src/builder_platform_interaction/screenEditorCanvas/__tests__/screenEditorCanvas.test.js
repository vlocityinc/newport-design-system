...
const selectors = {
    ...
    highlightFields:
        '.screen-editor-canvas-body builder_platform_interaction-screen-editor-highlight',
...
describe('help icon', () => {
    it('displays the help icon when there is help text', () => {
        ...
        return Promise.resolve().then(() => {
            const helpIcon = screenEditorCanvasElement.shadowRoot.querySelector(
...
describe('fields rendered', () => {
    ...
    it('Correct number of screen fields are rendered', () => {
        return Promise.resolve().then(() => {
            const highlightFields = screenEditorCanvasElement.shadowRoot.querySelectorAll(
                selectors.highlightFields
            );
...
describe('screen canvas', () => {
    it('displays error card when screen has errors', () => {
        ...
        return Promise.resolve().then(() => {
            const headerHighlight = screenEditorCanvasElement.shadowRoot.querySelector(
                selectors.highlightElementSlot
            );
            const screenFieldCard = headerHighlight.querySelector(
                selectors.screenFieldCard
            );
            expect(screenFieldCard).not.toBeNull();
            const cardBody = screenFieldCard.shadowRoot.querySelector(
                selectors.screenFieldCardBody
            );
            expect(cardBody).not.toBeNull();
            expect(cardBody.textContent).toEqual(
                'FlowBuilderScreenEditor.invalidScreen'
            );
        });
    });
    ...
    it('displays flow label when screen has no error and flow label is defined', () => {
        ...
        return Promise.resolve().then(() => {
            ...
            const screenCanvasHeader = headerHighlight.querySelector(
                selectors.screenCanvasHeader
            );
    ...
    it('displays default string when screen has no error and flow label is not defined', () => {
        ...
        return Promise.resolve().then(() => {
            ...
            expect(screenCanvasHeader.textContent).toEqual(
                '[FlowBuilderScreenEditor.screenTitlePlaceHolder]'
            );
        });
    });
});
...
describe('canvas elements draggability', () => {
    ...
    it('screen field is draggable', () => {
        return Promise.resolve().then(() => {
            const field = screenEditorCanvasElement.shadowRoot.querySelector(
                'div.screen-editor-canvas-body builder_platform_interaction-screen-editor-highlight'
            );
...
describe('Click handling on canvas', () => {
    ...
    it('clicking on the canvas fires correct event', () => {
        return Promise.resolve().then(() => {
            ...
            screenEditorCanvasElement.addEventListener(
                SCREEN_EDITOR_EVENT_NAME.SCREEN_ELEMENT_SELECTED,
                eventCallback
            );
            const canvasDiv = screenEditorCanvasElement.shadowRoot.querySelector(
...
describe('handleDragEnter', () => {
    ...
    it('dragEnter removes the expect class from the highlight', () => {
        return Promise.resolve().then(() => {
            ...
            const draggingRegion = screenEditorCanvasElement.shadowRoot.querySelector(
                selectors.draggingRegion
            );
            expect(draggingRegion.classList).toContain('slds-hide');

            // Fire dragEnter event on the canvas
            const canvasBody = screenEditorCanvasElement.shadowRoot.querySelector(
                selectors.canvasBody
            );
...
describe('onDrop', () => {
    ...
    beforeEach(() => {
        ...
        const fieldHighlights = screenEditorCanvasElement.shadowRoot
        ...
        for (const fieldHighlight of fieldHighlights) {
            fieldHighlight.getBoundingClientRect = boundingClientRectMock(
                counter
            );
            counter++;
        }
    ...
    it('dropping an element to the same location does not result in reorder', () => {
        return Promise.resolve().then(() => {
            ...
            dropEvent.dataTransfer.setData(
                'text',
                screenEditorCanvasElement.screen.fields[0].guid
            );

            const canvasBodyDiv = screenEditorCanvasElement.shadowRoot.querySelector(
                selectors.canvasContainer
            );
    ...
    it('dropping an element to a higher y coordinate should result in reorder event', () => {
        return Promise.resolve().then(() => {
            ...
            dropEvent.dataTransfer.setData(
                'text',
                screenEditorCanvasElement.screen.fields[2].guid
            );
