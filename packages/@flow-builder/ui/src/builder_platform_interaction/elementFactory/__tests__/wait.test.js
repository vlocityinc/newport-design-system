...
jest.mock('../base/baseElement', () => {
    return {
        ...
        baseCanvasElementsArrayToMap: jest.requireActual('../base/baseElement')
...
createConditionMetadataObject.mockImplementation(element =>
...
describe('wait', () => {
    describe('createWaitWithWaitEvents', () => {
        ...
        describe('wait events', () => {
            ...
            it('includes wait event for all wait event references present', () => {
                ...
                expect(wait.waitEvents[0].guid).toEqual(
                    waitEventReferences[0].waitEventReference
                );
                expect(wait.waitEvents[1].guid).toEqual(
                    waitEventReferences[1].waitEventReference
                );
                expect(wait.waitEvents[2].guid).toEqual(
                    waitEventReferences[2].waitEventReference
                );
            });
        });
    ...
    describe('createDuplicateWait function', () => {
        const {
            duplicatedElement,
            duplicatedChildElements
        } = createDuplicateWait({}, 'duplicatedGuid', 'duplicatedName', {}, {});
        ...
        it('duplicatedElement has updated defaultConnectorLabel', () => {
            expect(duplicatedElement.defaultConnectorLabel).toEqual(
                LABELS.emptyDefaultWaitPathLabel
            );
        });
    ...
    describe('createWaitEvent', () => {
        ...
        it('has NO_CONDITIONS as the default condition logic', () => {
            ...
            expect(waitEvent.conditionLogic).toEqual(
                defaultWaitEvent.conditionLogic
            );
        });
        ...
        it('calls createCondition for every condition given if condition logic is not NO_CONDITIONS', () => {
            ...
            expect(waitEvent.conditions[0]).toBe(
                createCondition.mock.results[0].value
            );
        ...
        it('sets the output parameters with additional parameters needed for absolute time event', () => {
            ...
            expect(createOutputParameter.mock.calls[0][0]).toEqual(
                mockWaitEvent.outputParameters[0]
            );
            expect(createOutputParameter.mock.calls[1][0]).toEqual(
                expectedAdditionalOutputParams[0]
            );

            expect(Object.keys(waitEvent.outputParameters)).toHaveLength(2);
            expect(
                waitEvent.outputParameters[
                    WAIT_TIME_EVENT_PARAMETER_NAMES.RESUME_TIME
                ]
            ).toEqual(createOutputParameter.mock.results[0].value);
            expect(
                waitEvent.outputParameters[
                    WAIT_TIME_EVENT_PARAMETER_NAMES.EVENT_DELIVERY_STATUS
                ]
            ).toEqual(createOutputParameter.mock.results[1].value);
        });
        ...
        it('sets the input parameters with additional parameters needed for direct record type event', () => {
            ...
            let index = 0;
            [
                ...mockWaitEvent.inputParameters,
                ...expectedAdditionalInputParameters
            ].forEach(inputParameter => {
                expect(createInputParameter.mock.calls[index++][0]).toEqual(
                    inputParameter
                );
            });
            ...
            waitEvent.inputParameters.forEach(inputParameter => {
                expect(inputParameter).toEqual(
                    createInputParameter.mock.results[index++].value
                );
            });
        });
        ...
        it('sets the input and output parameters for a platform event type', () => {
            ...
            expect(createInputParameter.mock.calls[0][0]).toEqual(
                mockWaitEvent.inputParameters[0]
            );
            expect(createInputParameter.mock.calls[1][0]).toEqual(
                mockWaitEvent.inputParameters[1]
            );

            expect(waitEvent.inputParameters).toHaveLength(2);
            expect(waitEvent.inputParameters[0]).toEqual(
                createInputParameter.mock.results[0].value
            );
            expect(waitEvent.inputParameters[1]).toEqual(
                createInputParameter.mock.results[1].value
            );
            ...
            expect(waitEvent.outputParameters[platformEventName]).toEqual(
    ...
    describe('createWaitWithWaitEventReferencesWhenUpdatingFromPropertyEditor', () => {
        ...
        it('element type is WAIT_WITH_MODIFIED_AND_DELETED_WAIT_EVENTS', () => {
            ...
            expect(result.elementType).toEqual(
                ELEMENT_TYPE.WAIT_WITH_MODIFIED_AND_DELETED_WAIT_EVENTS
            );
        });
        ...
        describe('new/modified waitEvents', () => {
            ...
            it('wait includes waitEvent references for all waitEvents present', () => {
                ...
                expect(result.canvasElement.waitEventReferences).toHaveLength(
                    3
                );
                expect(
                    result.canvasElement.waitEventReferences[0]
                        .waitEventReference
                ).toEqual(waitEvents[0].guid);
                expect(
                    result.canvasElement.waitEventReferences[1]
                        .waitEventReference
                ).toEqual(waitEvents[1].guid);
                expect(
                    result.canvasElement.waitEventReferences[2]
                        .waitEventReference
                ).toEqual(waitEvents[2].guid);
            });
            ...
            it('includes waitEvents for all waitEvents present', () => {
                ...
                expect(result.childElements[0].guid).toEqual(
                    waitEvents[0].guid
                );
                expect(result.childElements[1].guid).toEqual(
                    waitEvents[1].guid
                );
                expect(result.childElements[2].guid).toEqual(
                    waitEvents[2].guid
                );
            });
        ...
        describe('deleted waitEvents', () => {
            ...
            it('wait does not include waitEvent references for deleted waitEvents', () => {
                ...
                expect(
                    result.canvasElement.waitEventReferences[0]
                        .waitEventReference
                ).toEqual(waitFromPropertyEditor.waitEvents[0].guid);
            });
    ...
    describe('createWaitWithWaitEventReferences', () => {
        ...
        describe('defaultConnectorLabel', () => {
            it('defaults to LABELS.emptyDefaultWaitPathLabel', () => {
                ...
                expect(wait.defaultConnectorLabel).toEqual(
                    LABELS.emptyDefaultWaitPathLabel
                );
            });
            it('is used if passed in', () => {
        ...
        describe('waitEvents', () => {
            it('wait includes waitEvents for all waitEvents present', () => {
                ...
                expect(wait.waitEventReferences[0].waitEventReference).toEqual(
                    waitFromFlow.waitEvents[0].guid
                );
                expect(wait.waitEventReferences[1].waitEventReference).toEqual(
                    waitFromFlow.waitEvents[1].guid
                );
                expect(wait.waitEventReferences[2].waitEventReference).toEqual(
                    waitFromFlow.waitEvents[2].guid
                );
            });
            ...
            it('are included in element map for all waitEvents present', () => {
                ...
                expect(
                    result.elements[waitFromFlow.waitEvents[0].guid]
                ).toMatchObject(waitFromFlow.waitEvents[0]);
                expect(
                    result.elements[waitFromFlow.waitEvents[1].guid]
                ).toMatchObject(waitFromFlow.waitEvents[1]);
                expect(
                    result.elements[waitFromFlow.waitEvents[2].guid]
                ).toMatchObject(waitFromFlow.waitEvents[2]);
            });
        });
        describe('wait connections', () => {
            let result;
    ...
    describe('createWaitMetadataObject', () => {
        ...
        describe('waitEvents', () => {
            it('wait includes waitEvents for all waitEvent references present', () => {
                ...
                expect(wait.waitEvents[1].guid).toEqual(
                    waitFromStore.waitEventReferences[1].waitEventReference
                );
                expect(wait.waitEvents[2].guid).toEqual(
                    waitFromStore.waitEventReferences[2].waitEventReference
                );
            });
            ...
            describe('conditions', () => {
                it('calls createConditionMetadataObject for each condition given', () => {
                    ...
                    expect(createConditionMetadataObject).toHaveBeenCalledTimes(
                        1
                    );
                    expect(createConditionMetadataObject).toHaveBeenCalledWith(
                        mockCondition
                    );
                    expect(wait.waitEvents[0].conditions).toHaveLength(1);
                    expect(wait.waitEvents[0].conditions[0]).toEqual(
                        mockCondition
                    );
                    expect(wait.waitEvents[0].conditions[0]).toBe(
                ...
                it('sets the condition logic to AND when no conditions exist', () => {
                    ...
                    expect(wait.waitEvents[0].conditionLogic).toEqual(
                        CONDITION_LOGIC.AND
                    );
                    expect(wait.waitEvents[1].conditionLogic).toEqual(
                        CONDITION_LOGIC.AND
                    );
                    expect(wait.waitEvents[2].conditionLogic).toEqual(
                        CONDITION_LOGIC.AND
                    );
                });
            ...
            it('sets the input parameters with result of createParameterItemMetadataObject for every input parameter given', () => {
                ...
                wait.waitEvents[0].inputParameters.forEach(inputParameter => {
                    expect(
                        createInputParameterMetadataObject.mock.calls[
                            index++
                        ][0]
                    ).toEqual(inputParameter);
                });
                ...
                wait.waitEvents[0].inputParameters.forEach(inputParameter => {
                    expect(
                        createInputParameterMetadataObject.mock.results[index++]
                            .value
                    ).toEqual(inputParameter);
                });
            });
            ...
            it('sets the output parameters with result of createParameterItemMetadataObject for output parameters with non empty values', () => {
                ...
                expect(
                    createOutputParameterMetadataObject.mock.calls[0][0]
                ).toEqual(wait.waitEvents[0].outputParameters[0]);
                expect(
                    createOutputParameterMetadataObject.mock.calls[1][0]
                ).toEqual(wait.waitEvents[0].outputParameters[1]);
