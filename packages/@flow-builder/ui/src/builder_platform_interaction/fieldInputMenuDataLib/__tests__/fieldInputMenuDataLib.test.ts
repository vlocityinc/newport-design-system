...
import { getResourceCategory, getResourceName } from '../fieldInputMenuDataLib';
...
describe('Tests for getResourceCategory', () => {
    it('Automatic output associated with "Create Records" should be under Record Create category', () => {
        expect(
            getResourceCategory(createElement(ELEMENT_TYPE.RECORD_CREATE, FLOW_DATA_TYPE.STRING.value, false))
        ).toEqual(LABELS.recordCreate);
    });

    it('for automatic output associated with "Loops" should be under Loop category', () => {
        expect(getResourceCategory(createElement(ELEMENT_TYPE.LOOP, FLOW_DATA_TYPE.STRING.value, false, true))).toEqual(
            LABELS.loops
        );
    });
    ...
    it('Automatic output associated with "Get Records" should be under Record Lookup category', () => {
        expect(
            getResourceCategory(createElement(ELEMENT_TYPE.RECORD_LOOKUP, FLOW_DATA_TYPE.SOBJECT.value, false, true))
    ...
    it('Collection Automatic output associated with "Get Records" should be under Record Lookup category', () => {
        expect(
            getResourceCategory(createElement(ELEMENT_TYPE.RECORD_LOOKUP, FLOW_DATA_TYPE.SOBJECT.value, true, true))
        ).toEqual(LABELS.recordLookup);
    });

    it('Subflows with manually assigned outputs should be under Subflow category', () => {
        expect(
            getResourceCategory(createElement(ELEMENT_TYPE.SUBFLOW, FLOW_DATA_TYPE.BOOLEAN.value, false, true))
        ).toEqual(LABELS.subflows);
    });

    it('Assignment should be under Assignments category', () => {
        expect(getResourceCategory(createElement(ELEMENT_TYPE.ASSIGNMENT))).toEqual(LABELS.assignments);
    });

    it('Variable should be under Simple Variables category', () => {
        expect(
            getResourceCategory(createElement(ELEMENT_TYPE.VARIABLE, FLOW_DATA_TYPE.NUMBER.value, false, false))
        ).toEqual(LABELS.simpleVariables);
    });

    it('Collection variable for non complex types should be under Simple collections category', () => {
        expect(
            getResourceCategory(createElement(ELEMENT_TYPE.VARIABLE, FLOW_DATA_TYPE.NUMBER.value, true, false))
        ).toEqual(LABELS.simpleCollections);
    });

    it('Record collections variable should be under Record Collections category', () => {
        expect(
            getResourceCategory(createElement(ELEMENT_TYPE.VARIABLE, FLOW_DATA_TYPE.SOBJECT.value, true, false))
        ).toEqual(LABELS.recordCollections);
    });

    it('Apex collection variable should be under Apex Collections category', () => {
        expect(
            getResourceCategory(createElement(ELEMENT_TYPE.VARIABLE, FLOW_DATA_TYPE.APEX.value, true, false))
        ).toEqual(LABELS.apexCollections);
    });

    it('Record variable should be under Record Variables category', () => {
        expect(
            getResourceCategory(createElement(ELEMENT_TYPE.VARIABLE, FLOW_DATA_TYPE.SOBJECT.value, false, false))
        ).toEqual(LABELS.recordVariables);
    });

    it('Apex variable should be under Apex Variable category', () => {
        expect(
            getResourceCategory(createElement(ELEMENT_TYPE.VARIABLE, FLOW_DATA_TYPE.APEX.value, false, false))
