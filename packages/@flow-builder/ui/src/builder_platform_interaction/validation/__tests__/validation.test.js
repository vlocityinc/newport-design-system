    ...
    describe('when props set to NAME', () => {
        ...
        it('should return an error if length is > 80 characters', () => {
            expect(
                validation.validateProperty(
                    'name',
                    'OJqlWSveOtulUjcyHgrDOOSPArDKdbftmvEKPBPDxLqrwtseblHPBcgctlMYmRsbPyngaEmZqCqMxksyv'
                )
            ).toBe(ValidationRules.LABELS.maximumCharactersLimit);
        });
    });
    ...
    it('returns the object with errors when rules fail at various level of properties', () => {
        ...
        const testObj = {
            ...
            outcomes: [
                {
                    name: {
                        value: 'valueWithMaximumCharLimitExceeded',
                        error: null
                    },
        ...
        const expectedObjectAfterValidateAll = {
            name: {
                value: 'valueWithError(trailingSpaces)_',
                error: TRAILING_UNDERSCORE_ERROR
            },
            label: { value: 'valueWithNoErrors', error: null },
            outcomes: [
                {
                    name: {
                        value: 'valueWithMaximumCharLimitExceeded',
                        error: ValidationRules.LABELS.maximumCharactersLimit
                    },
                    devName: { value: 'mockValue', error: null },
                    conditions: [
                        {
                            leftHandSide: {
                                value: '',
                                error: ValidationRules.LABELS.cannotBeBlank
                            }
                        },
                        {
                            leftHandSide: {
                    ...
                    conditions: [
                        {
                            ...
                            rightHandSide: {
                                value: 'valueWithError',
                                error: 'LHS is valueWithNoErrors'
                            }
                        }
                    ]
                },
