...
describe('screen-display-text-field-properties-editor', () => {
    ...
    it('Name field should be a string type input', async () => {
        await ticks(1);
        const nameField = query(screenDisplayTextFieldPropEditor, SELECTORS.NAME_FIELD);
        expect(nameField.type).toBe('string');
    });
    it('Name field should be filled in with the display text field name', async () => {
        await ticks(1);
    ...
    it('Value of display field is rendered in the input field', async () => {
        await ticks(1);
    ...
    it('Property changed on name field', async () => {
        await ticks(1);
        const rederedNameField = query(screenDisplayTextFieldPropEditor, SELECTORS.NAME_FIELD);
    ...
    it('Property changed on display field', async () => {
        await ticks(1);
