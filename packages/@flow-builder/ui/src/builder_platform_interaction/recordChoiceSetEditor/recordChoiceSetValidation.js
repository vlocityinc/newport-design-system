...
const validateFilter = () => {
    return filter => {
        ...
        if (filter[EXPRESSION_PROPERTY_TYPE.LEFT_HAND_SIDE].value) {
            rules[EXPRESSION_PROPERTY_TYPE.OPERATOR] = [
                ValidationRules.shouldNotBeBlank
            ];
...
const validateOutputAssignments = () => {
    return outputAssignment => {
        ...
        if (outputAssignment[EXPRESSION_PROPERTY_TYPE.RIGHT_HAND_SIDE].value) {
            rules[EXPRESSION_PROPERTY_TYPE.LEFT_HAND_SIDE] = [
                ValidationRules.shouldNotBeBlank
            ];
        }
...
export const getRules = (recordChoice, showSecondSection) => {
    const overrideRules = Object.assign(
        {},
        recordChoiceSetValidation.finalizedRules
    );
    overrideRules.object.push(
        ValidationRules.validateResourcePicker(recordChoice.objectIndex)
    );
    if (
        recordChoice.filterType &&
        recordChoice.filterType.value === RECORD_FILTER_CRITERIA.ALL
    ) {
