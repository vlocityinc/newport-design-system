...
type BreadcrumbView = FieldInputBreadcrumb & { displayText: string };
...
export const setFocusOnBreadcrumbLink = (activeElement: any, previousNextElement: ELEMENT_SIBLINGS) => {
    const breadcrumbListItem = activeElement?.parentElement;
    if (breadcrumbListItem) {
        breadcrumbListItem[previousNextElement]?.firstElementChild?.focus();
    }
};
 ...
 */
export const mapToDisplayField = (breadcrumb: FieldInputBreadcrumb) =>
 ...
 */
export const computeViewAttributes = (breadcrumb: FieldInputBreadcrumb): BreadcrumbView => ({
    ...breadcrumb,
