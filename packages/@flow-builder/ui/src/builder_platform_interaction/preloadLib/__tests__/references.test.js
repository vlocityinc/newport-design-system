...
jest.mock('builder_platform_interaction/mergeFieldLib', () => ({
    resolveReferenceFromIdentifier: jest
        .fn()
...
describe('loadReferencesIn', () => {
    it('resolves references used in an element', async () => {
        ...
        expect(resolveReferenceFromIdentifier.mock.calls[0][0]).toBe(
            stringVariable.guid
        );
        expect(resolveReferenceFromIdentifier.mock.calls[1][0]).toBe(
            numberVariable.guid
        );
    });
    ...
    it('returns a rejected Promise if some references cannot be resolved because a server call failed', async () => {
        resolveReferenceFromIdentifier.mockImplementationOnce(() =>
            Promise.reject('Server called failed')
        );
        await expect(loadReferencesIn(assignmentElement)).rejects.toEqual(
