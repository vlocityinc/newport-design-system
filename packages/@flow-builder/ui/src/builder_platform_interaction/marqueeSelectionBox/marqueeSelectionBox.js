...
export default class MarqueeSelectionBox extends LightningElement {
    ...
    get getBoxLocationAndDimension() {
        const boxLeft = Math.min(
            this.marqueeStartPoint[0],
            this.marqueeEndPoint[0]
        );
        const boxTop = Math.min(
            this.marqueeStartPoint[1],
            this.marqueeEndPoint[1]
        );
        const boxWidth = Math.abs(
            this.marqueeEndPoint[0] - this.marqueeStartPoint[0]
        );
        const boxHeight = Math.abs(
            this.marqueeEndPoint[1] - this.marqueeStartPoint[1]
        );
