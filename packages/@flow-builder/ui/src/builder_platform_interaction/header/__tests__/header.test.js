...
describe('HEADER', () => {
    ...
    it('checks the rendering FLOW NAME VERSION TITLE', () => {
        ...
        return Promise.resolve().then(() => {
            expect(headerComponent.shadowRoot.querySelector(selectors.flowNameVersionTitle).title).toEqual(`Flow Name${LABELS.versionLabelText}1`);
        });
    });
    ...
    it('checks the rendering FLOW VERSION', () => {
        const headerComponent = createComponentForTest('Flow Name', '1');
        return Promise.resolve().then(() => {
            expect(headerComponent.shadowRoot.querySelector(selectors.flowVersion).textContent).toEqual(`${LABELS.versionLabelText}1`);
        });
    });
    ...
    it('checks the rendering BACK label', () => {
        const headerComponent = createComponentForTest();
        return Promise.resolve().then(() => {
            expect(headerComponent.shadowRoot.querySelector(selectors.backLabel).textContent).toEqual(LABELS.backButtonText);
        });
    });
    ...
    it('checks the rendering HELP label', () => {
        const headerComponent = createComponentForTest();
        return Promise.resolve().then(() => {
            expect(headerComponent.shadowRoot.querySelector(selectors.helpLabel).textContent).toEqual(LABELS.helpButtonText);
        });
    });
