...
const ITEMS = [
    {
        ...
        label: 'Screen Flow',
        description:
            'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Aenean bibendum nulla dignissim, pellentesque tortor ut, vestibulum urna. Cras convallis facilisis sodales. In hac habitasse platea dictumst. Morbi ut tempor sem.',
        itemId: 'Flow1'
    },
    {
        isSelected: false,
...
describe('visual-picker-list', () => {
    ...
    it('should fire VisualPickerListChangedEvent when selecting another item', async () => {
        ...
        expect(eventCallback.mock.calls[0][0].detail).toEqual({
            items: [
                { id: ITEMS[0].itemId, isSelected: false },
                { id: visualPickerItem.itemId, isSelected: true }
            ]
        });
    });
    ...
    it('allows multiple selections when selecting another item', async () => {
        ...
        expect(eventCallback).toHaveBeenCalled();
        expect(eventCallback.mock.calls[0][0].detail).toEqual({
            items: [{ id: visualPickerItems[1].itemId, isSelected: true }]
        });
    });
