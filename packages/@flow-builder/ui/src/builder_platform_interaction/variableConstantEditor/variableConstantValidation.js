...
export const getRules = variableConstantResource => {
    ...
    if (isComplexType(getValueFromHydratedItem(variableConstantResource.dataType))) {
        overrideRules.subtype = [
            ...
            ValidationRules.validateResourcePicker(variableConstantResource.subtypeIndex)
        ];
    }

    if (
        variableConstantResource.dataType &&
        !isComplexType(getValueFromHydratedItem(variableConstantResource.dataType)) &&
        variableConstantResource.dataType.value !== FLOW_DATA_TYPE.PICKLIST &&
        variableConstantResource.dataType.value !== FLOW_DATA_TYPE.MULTI_PICKLIST &&
        !variableConstantResource.isCollection
    ) {
        overrideRules.defaultValue = [
            ValidationRules.validateResourcePicker(variableConstantResource.defaultValueIndex)
        ];
    }
