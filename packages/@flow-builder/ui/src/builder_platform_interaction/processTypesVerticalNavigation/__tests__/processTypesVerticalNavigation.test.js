...
describe('process-types-vertical-navigation', () => {
        ...
        describe('Process types details', () => {
            ...
            test('number of "other" process types', () => {
                expect(processTypesVerticalNavigation.otherProcessTypes).toHaveLength(MOCK_PROCESS_TYPES.OTHERS.length);
            });
            ...
            test('details of "other" process types (API)', () => {
                expect(processTypesVerticalNavigation.otherProcessTypes).toEqual(expect.arrayContaining([
                    expect.objectContaining({ label: 'Checkout Flow', name: FLOW_PROCESS_TYPE.CHECKOUT_FLOW, iconName: PROCESS_TYPES_ICONS.OTHERS.get(FLOW_PROCESS_TYPE.CHECKOUT_FLOW)}),
                    expect.objectContaining({ label: 'Contact Request Flow', name: FLOW_PROCESS_TYPE.CONTACT_REQUEST_FLOW, iconName: PROCESS_TYPES_ICONS.OTHERS.get(FLOW_PROCESS_TYPE.CONTACT_REQUEST_FLOW)}),
                    expect.objectContaining({ label: 'Embedded Appointment Management Flow', name: FLOW_PROCESS_TYPE.FIELD_SERVICE_WEB, iconName: PROCESS_TYPES_ICONS.OTHERS.get(FLOW_PROCESS_TYPE.FIELD_SERVICE_WEB)}),
                    expect.objectContaining({ label: 'Field Service Mobile Flow', name: FLOW_PROCESS_TYPE.FIELD_SERVICE_MOBILE, iconName: PROCESS_TYPES_ICONS.OTHERS.get(FLOW_PROCESS_TYPE.FIELD_SERVICE_MOBILE)}),
            ...
            test('process types name', () => {
                ...
                const joinedProcessTypesNames = Array.from(processTypesVerticalNavigationItemIcons).map(itemIcon => {
                    return itemIcon.name;
                }).join('');
                expect(joinedProcessTypesNames).toBe(ALL_PROCESS_TYPE.name + MOCK_ALL_PROCESS_TYPES.map(processType => {
        ...
        describe('Events', () => {
            test('should fire "ProcessTypeSelectedEvent" when selecting a process type with process type name', async () => {
                ...
                const processTypesVerticalNavigationItemIcon = getAllVerticalNavigationItemIcons(processTypesVerticalNavigation)[1];
                const autolaunchedFlowAnchor = getProcessTypeAnchor(processTypesVerticalNavigationItemIcon);
                autolaunchedFlowAnchor.dispatchEvent(new Event('click'));
                await Promise.resolve();
                expect(eventCallback).toHaveBeenCalled();
                expect(eventCallback.mock.calls[0][0].detail).toEqual({name: FLOW_PROCESS_TYPE.AUTO_LAUNCHED_FLOW});
            });
    });
});