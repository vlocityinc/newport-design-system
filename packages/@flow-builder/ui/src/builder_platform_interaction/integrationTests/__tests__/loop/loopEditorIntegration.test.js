...
describe('Loop Editor', () => {
    ...
    describe('loop variable', () => {
        it('is correctly loaded, with no error, for an existing loop', () => {
            ...
            return resolveRenderCycles(() => {
                ...
                expect(loopVariableLightningCombobox.inputText).toBe('{!' + stringVariable.name + '}');
        ...
        it('is enabled after the collection variable is set to a valid value', () => {
            ...
            return resolveRenderCycles(() => {
                const colVariableLightningCombobox = getCollectionVariableComboboxElement(loopElement);
                colVariableLightningCombobox.dispatchEvent(textInputEvent('{!' + stringCollectionVariable1.name + '}'));
                return resolveRenderCycles(() => {
            ...
            return resolveRenderCycles(() => {
                ...
                return resolveRenderCycles(() => {
                    ...
                    return resolveRenderCycles(() => {
                        ...
                        expect(loopVariableLightningCombobox.items[1].label).toBe(
                            'FLOWBUILDERELEMENTCONFIG.VARIABLEPLURALLABEL'
                        );
                        expect(loopVariableLightningCombobox.items[1].items).toHaveLength(1);
                        expect(loopVariableLightningCombobox.items[1].items[0].text).toBe(stringVariable.name);
                        expect(loopVariableLightningCombobox.items[1].items[0].subText).toBe('String');
                        expect(loopVariableLightningCombobox.items[1].items[0].displayText).toBe(
                            '{!' + stringVariable.name + '}'
                        );
                        expect(loopVariableLightningCombobox.items[1].items[0].value).toBe(stringVariable.guid);
                    });
                });
            });
        });
        it('shows an error when its data type is different from the data type in the collection variable', () => {
            ...
            return resolveRenderCycles(() => {
                ...
                return resolveRenderCycles(() => {
                    ...
                    colVariableLightningCombobox.dispatchEvent(
                        textInputEvent('{!' + accountSObjectCollectionVariable.name + '}')
                    );
                    return resolveRenderCycles(() => {
                        colVariableLightningCombobox.dispatchEvent(blurEvent);
                        return resolveRenderCycles(() => {
            ...
            return resolveRenderCycles(() => {
                ...
                return resolveRenderCycles(() => {
                    ...
                    return resolveRenderCycles(() => {
                        ...
                        expect(loopVariableLightningCombobox.items[1].items).toHaveLength(4);
                        ...
                        expect(loopVariableLightningCombobox.items[1].items[3]).toMatchObject({
                            dataType: 'SObject',
                            subtype: 'Account',
        ...
        it('shows an error when its sObject type is different from the sObject type in the collection variable', () => {
            ...
            return resolveRenderCycles(() => {
                ...
                return resolveRenderCycles(() => {
                    ...
                    colVariableLightningCombobox.dispatchEvent(
                        textInputEvent('{!' + caseSObjectCollectionVariable.name + '}')
                    );
                    return resolveRenderCycles(() => {
                        colVariableLightningCombobox.dispatchEvent(blurEvent);
                        return resolveRenderCycles(() => {
                    ...
                    return resolveRenderCycles(() => {
                        ...
                        return resolveRenderCycles(() => {
                            loopVariableLightningCombobox.dispatchEvent(
                                textInputEvent('{!' + accountSObjectVariable.name + '}')
                            );
                            loopVariableLightningCombobox.dispatchEvent(blurEvent);
                            return resolveRenderCycles(() => {
                                expect(loopVariableLightningCombobox.validity).toBeFalsy();
    ...
    describe('collection variable', () => {
        it('is correctly loaded, with no error, for an existing loop', () => {
            ...
            return resolveRenderCycles(() => {
                const colVariableLightningCombobox = getCollectionVariableComboboxElement(loopElement);
                expect(colVariableLightningCombobox.inputText).toBe('{!' + stringCollectionVariable1.name + '}');
        ...
        it('shows only collections in dropdown', () => {
            ...
            return resolveRenderCycles(() => {
                ...
                return resolveRenderCycles(() => {
                    ...
                    // mock-store data
                    const apexCollectionVariables = colVariableLightningCombobox.items[1];
                    expect(apexCollectionVariables.label).toBe(
                        'FLOWBUILDERELEMENTCONFIG.APEXCOLLECTIONVARIABLEPLURALLABEL'
                    );
                    const apexCollectionVariablesItems = apexCollectionVariables.items;
                    expect(apexCollectionVariablesItems).toHaveLength(3);
                    expect(apexCollectionVariablesItems[0].text).toBe(apexComplexTypeCollectionVariable.name);

                    // There are 4 non-sObject variables with 'isCollection:
                    // true' in the mock-store data
                    const nonSobjectCollectionVariables = colVariableLightningCombobox.items[2];
                    const nonSobjectCollectionVariablesItems = nonSobjectCollectionVariables.items;
                    ...
                    expect(nonSobjectCollectionVariablesItems[0].text).toBe(dateCollectionVariable.name);

                    // There are 4 sObject collection variables and 1
                    // lookupRecord in automatic output handling mode in the
                    // mock-store data
                    const sobjectCollectionVariables = colVariableLightningCombobox.items[3];
                    const sobjectCollectionVariablesitems = sobjectCollectionVariables.items;
                    ...
                    expect(sobjectCollectionVariablesitems[0].text).toBe(accountSObjectCollectionVariable.name);
                    expect(sobjectCollectionVariablesitems[0].subText).toBe('Account');
                });
            });
        });
