...
import {
    ...
    focusoutEvent, textInputEvent, blurEvent, auraFetch
...
const getInvocableActionParameters = (invocableActionParameters) => params => {
    ...
    return invocableActionParametersForAction ? ({ data : invocableActionParametersForAction }) : ({ error : 'Cannot find invocable action parameters'});
};
...
describe('Invocable Action Editor', () => {
    ...
    describe('Flow with an apex action with all types', () => {
        beforeAll(() => {
            setAuraFetch(auraFetch({
                'c.getAllInvocableActionsForType' : () => ({ data : mockActions }),
                'c.getInvocableActionParameters' : getInvocableActionParameters({ 'apex' : { 'AllTypesApexAction' : mockAllTypesActionParameters } })
            }));
    ...
    describe('Flow with an action that has a text collection input parameter set to a text collection variable', () => {
        beforeAll(() => {
            setAuraFetch(auraFetch({
                'c.getAllInvocableActionsForType' : () => ({ data : mockActions }),
                'c.getInvocableActionParameters' : getInvocableActionParameters({ 'submit' : { 'submit' : submitForApprovalActionParameters } })
            }));
