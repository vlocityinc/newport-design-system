...
import { LIGHTNING_COMPONENTS_SELECTORS,
    ...
    getEntityResourcePicker, auraFetch,
    getRadioGroup, getChildComponent, changeInputValue, changeComboboxValue, newFilterItem} from "../integrationTestUtils";
...
const getEntityResourcePickerComboboxElement = (entityResourcePicker) => {
    const resourcePicker = getShadowRoot(entityResourcePicker).querySelector(INTERACTION_COMPONENTS_SELECTORS.BASE_RESOURCE_PICKER);
    const combobox = getShadowRoot(resourcePicker).querySelector(INTERACTION_COMPONENTS_SELECTORS.COMBOBOX);
    const lightningGroupCombobox = getShadowRoot(combobox).querySelector(LIGHTNING_COMPONENTS_SELECTORS.LIGHTNING_GROUPED_COMBOBOX);
...
const getResourceGroupedCombobox = (editor) => {
    ...
    const ferovResourcePicker = getShadowRoot(sObjectOrSObjectCollectionPicker).querySelector(INTERACTION_COMPONENTS_SELECTORS.FEROV_RESOURCE_PICKER);
...
describe('Record Lookup Editor', () => {
    ...
    describe('name and dev name', () => {
        ...
        it('display error if name is cleared', () => {
            ...
            changeInputValue(labelInput, newLabel);
            return resolveRenderCycles(() => {
                expect(recordLookupElement.node.label.error).toBe(FLOW_BUILDER_VALIDATION_ERROR_MESSAGES.CANNOT_BE_BLANK);
            });
        });
        it('display error if devName is cleared', () => {
            ...
            changeInputValue(devNameInput, newDevName);
            return resolveRenderCycles(() => {
                expect(recordLookupElement.node.name.error).toBe(FLOW_BUILDER_VALIDATION_ERROR_MESSAGES.CANNOT_BE_BLANK);
            });
        });
    });
        ...
        describe('Working with sObject', () => {
            ...
            it('remove selected entity should hide filter, sort, store option and query fields', () => {
                ...
                return resolveRenderCycles(() => {
                    ...
                    expect(recordLookupElement.node.object.error).toBe(FLOW_BUILDER_VALIDATION_ERROR_MESSAGES.CANNOT_BE_BLANK);
                });
            });
            ...
            describe('Filter', () => {
                ...
                it('LHS/Operator/LHS values', () => {
                    expect(recordFilter.filterItems[0]).toMatchObject(newFilterItem("Account.BillingCity", "EqualTo", "San Francisco", "String"));
                });
            });
        ...
        describe('Working with fields', () => {
            ...
            describe('Output Assignments', () => {
                ...
                it('LHS/Operator/LHS values', () => {
                    ...
                    expect(baseExpressionBuilder.lhsValue).toMatchObject({value: 'Account.BillingCity'});
