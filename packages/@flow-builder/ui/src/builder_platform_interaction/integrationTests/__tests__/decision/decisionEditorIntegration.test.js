...
const SELECTORS = {
    ...
    DEFAULT_OUTCOME:
        'builder_platform_interaction-label-description.defaultOutcome',
...
describeSkip('Decision Editor', () => {
    describe('label and description', () => {
        it('Adding name autofills dev name', () => {
            ...
            return resolveRenderCycles(() => {
                ...
                return resolveRenderCycles(() => {
                    ...
                    expect(decisionEditor.node.name.value).toBe(
                        NEW_DECISION_DEV_NAME
                    );
                });
            });
        });
        ...
        it('Dev name is unchanged if it already exists and name is modified', () => {
            const decisionEditor = createComponentForTest(
    ...
    describe('outcome list', () => {
        it('default outcome and detail page are always present', () => {
            ...
            return resolveRenderCycles(() => {
                ...
                const outcomeDetailPages = decisionEditor.shadowRoot.querySelectorAll(
                    SELECTORS.OUTCOME
                );
                expect(outcomeDetailPages).toHaveLength(1);
                const outcomes = decisionEditor.querySelectorAll(
                    SELECTORS.VERTICAL_TAB_NAV_ITEM
                );
        ...
        it('click on Add Outcome will create a new outcome', () => {
            ...
            return resolveRenderCycles(() => {
                const addOutcomeElement = decisionEditor.shadowRoot.querySelector(
        ...
        it('reorder list sets the correct order of outcomes', () => {
            ...
            return resolveRenderCycles(() => {
                expect(decisionEditor.getNode().outcomes[0].guid).toBe(
                    outcome1Guid
                );
                const reorderableNav = decisionEditor.shadowRoot.querySelector(
                    SELECTORS.REORDERABLE_NAV
                );
    ...
    describe('outcome detail', () => {
        describe('label and description', () => {
            it('Adding name autofills dev name', () => {
                const decisionEditor = createComponentForTest(emptyDecision);
                return resolveRenderCycles(() => {
                    const outcomeDetailPage = decisionEditor.shadowRoot.querySelector(
                        SELECTORS.OUTCOME
                    );
                    const detailLabelInput = unwrap(
                        outcomeDetailPage.shadowRoot.querySelector(
                    ...
                    return resolveRenderCycles(() => {
                        expect(
                            decisionEditor.node.outcomes[0].label.value
                        ).toBe(NEW_OUTCOME_LABEL);
                        expect(decisionEditor.node.outcomes[0].name.value).toBe(
                            NEW_OUTCOME_DEV_NAME
                        );
                    });
                });
            });
        ...
        describe('add/delete conditions', () => {
            it('add condition', () => {
                ...
                return resolveRenderCycles(() => {
                    ...
                    const conditionList = outcomeDetailPage.shadowRoot.querySelector(
                        SELECTORS.CONDITION_LIST
                    );
                    const list = conditionList.shadowRoot.querySelector(
                        SELECTORS.LIST
                    );
                    // Should only have 1 condition in the list
                    expect(
                        decisionEditor.getNode().outcomes[0].conditions
                    ).toHaveLength(1);

                    const button = list.shadowRoot.querySelector(
            ...
            it('delete is disabled for one condition', () => {
                ...
                return resolveRenderCycles(() => {
                    ...
                    const button = row.shadowRoot.querySelector(
                        SELECTORS.LIGHTNING_BUTTON_ICON
                    );
            ...
            it('delete is clickable and works for 2 or more conditions', () => {
                ...
                return resolveRenderCycles(() => {
                    const outcomeDetailPage = decisionEditor.querySelector(
                        SELECTORS.OUTCOME
                    );
                    const list = outcomeDetailPage.querySelector(
                        SELECTORS.LIST
                    );
                    const conditionList = outcomeDetailPage.shadowRoot.querySelector(
                        SELECTORS.CONDITION_LIST
                    );
                    const addConditionButton = list.shadowRoot.querySelector(
                        SELECTORS.LIGHTNING_BUTTON
                    );
                    ...
                    const row = conditionList.querySelector(SELECTORS.ROW);
                    const deleteButton = row.shadowRoot.querySelector(
                        SELECTORS.LIGHTNING_BUTTON_ICON
                    );
                    return resolveRenderCycles(() => {
        ...
        describe('logic', () => {
            it('all conditions are met has AND for more than one row', () => {
                const decisionEditor = createComponentForTest(
                    decisionWithOneOutcomeWithTwoConditions
                );
                return resolveRenderCycles(() => {
                    expect(
                        decisionEditor.node.outcomes[0].conditionLogic.value
                    ).toBe(CONDITION_LOGIC.AND);
                    ...
                    const legendText = row[1].shadowRoot.querySelector(
                        SELECTORS.LEGEND_TEXT
                    ).textContent;
            ...
            it('any condition is met has OR for more than one row', () => {
                ...
                return resolveRenderCycles(() => {
                    ...
                    const conditionCombobox = outcomeDetailPage.querySelector(
                        SELECTORS.CONDITION_COMBOBOX
                    );
                    ...
                    return resolveRenderCycles(() => {
                        expect(
                            decisionEditor.node.outcomes[0].conditionLogic.value
                        ).toBe(CONDITION_LOGIC.OR);
                        const conditionList = outcomeDetailPage.shadowRoot.querySelector(
                            SELECTORS.CONDITION_LIST
                        );
                        const row = conditionList.querySelectorAll(
                            SELECTORS.ROW
                        );
                        const legendText = row[1].shadowRoot.querySelector(
                            SELECTORS.LEGEND_TEXT
            ...
            it('custom condition logic input and shows number for rows', () => {
                ...
                return resolveRenderCycles(() => {
                    ...
                    const cbChangeEvent = changeEvent(
                    ...
                    return resolveRenderCycles(() => {
                        ...
                        let legendText = row[0].shadowRoot.querySelector(
                            SELECTORS.LEGEND_TEXT
                        ).textContent;
                        expect(legendText).toBe('1');
                        legendText = row[1].shadowRoot.querySelector(
                            SELECTORS.LEGEND_TEXT
                        ).textContent;
        ...
        describe('default outcome', () => {
            it('no delete outcome button', () => {
                ...
                return resolveRenderCycles(() => {
                    ...
                    return resolveRenderCycles(() => {
                        const deleteOutcomeButton = decisionEditor.shadowRoot.querySelector(
                            SELECTORS.LIGHTNING_BUTTON
                        );
            ...
            it('can update label', () => {
                ...
                return resolveRenderCycles(() => {
                    ...
                    return resolveRenderCycles(() => {
                        const labelDescription = decisionEditor.shadowRoot.querySelector(
                            SELECTORS.DEFAULT_OUTCOME
                        );
                        const defaultOutcomeLabelInput = labelDescription.shadowRoot.querySelector(
                            SELECTORS.LABEL
                        );
        ...
        describe('delete outcome', () => {
            it('delete outcome button is not present when only one outcome', () => {
                ...
                return resolveRenderCycles(() => {
                    ...
                    const deleteOutcomeButton = outcomeDetailPage.shadowRoot.querySelector(
                        SELECTORS.LIGHTNING_BUTTON
                    );
                    expect(deleteOutcomeButton).toBeNull();
                });
            ...
            it('delete outcome available with 2 or more outcomes and works', () => {
                ...
                return resolveRenderCycles(() => {
                    let outcomes = decisionEditor.querySelectorAll(
                        SELECTORS.VERTICAL_TAB_NAV_ITEM
                    );
                    expect(outcomes).toHaveLength(3);
                    const outcomeDetailPage = decisionEditor.shadowRoot.querySelector(
                    ...
                    return resolveRenderCycles(() => {
                        outcomes = decisionEditor.querySelectorAll(
                            SELECTORS.VERTICAL_TAB_NAV_ITEM
                        );
                        expect(outcomes).toHaveLength(2);
                    });
