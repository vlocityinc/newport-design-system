...
const getProcessTypesNavigationItems = (modalBody) => {
    const processTypeNavigation = modalBody.shadowRoot.querySelector(SELECTORS.PROCESS_TYPES_NAVIGATION);
    const verticalNavigation = processTypeNavigation.shadowRoot.querySelector('lightning-vertical-navigation');
    return verticalNavigation.querySelectorAll('lightning-vertical-navigation-item-icon');
};
...
const getTemplateList = (processTypeTemplates, section) => processTypeTemplates.shadowRoot.querySelector(section).querySelector(SELECTORS.VISUAL_PICKER_LIST);

const getTemplateItems = (templatesList) => templatesList.shadowRoot.querySelectorAll(SELECTORS.VISUAL_PICKER_ITEM);

const getExploreFlowTemplateTile = (processTypeTemplates) => processTypeTemplates.shadowRoot.querySelector(SELECTORS.TEMPLATES_SECTION).querySelector(SELECTORS.GET_TEMPLATES_TILE);
    ...
    describe('with existing templates', () => {
        beforeAll(() => {
            setAuraFetch(auraFetch({
                'c.getProcessTypes' : () => ({ data : {processTypes: JSON.stringify(MOCK_RAW_PROCESS_TYPES)} }),
        ...
        describe('Process Types', () => {
            ...
            it('should shows the correct number of process types in navigation', () => {
                ...
                expect(processTypesNavigationItems).toHaveLength(MOCK_RAW_PROCESS_TYPES.length + 1);
            });
            it('should shows the correct number of templates in navigation', () => {
                return resolveRenderCycles(() => {
                    ...
                    expect(templateItems).toHaveLength(MOCK_RAW_TEMPLATE_LIST.length);
                });
            });
            describe('select a Process Type', () => {
               it('should only display autolaunched flow templates', async () => {
                   const processTypesNavigationItems = getProcessTypesNavigationItems(newFlowModalBody);
                   getProcessTypeLink(processTypesNavigationItems, FLOW_PROCESS_TYPE.AUTO_LAUNCHED_FLOW).click();
                   ...
                   expect(getTemplateItemTitle(templateItems[0])).toBe('myFlowTemplateGetAccount');
                   expect(getTemplateItemTitle(templateItems[1])).toBe('myAutoLaunchedFlow');
               });
               it('should only display screen flow templates', async () => {
                   const processTypesNavigationItems = getProcessTypesNavigationItems(newFlowModalBody);
                   getProcessTypeLink(processTypesNavigationItems, FLOW_PROCESS_TYPE.FLOW).click();
                   await Promise.resolve();
                   const processTypesTemplates = getProcessTypesTemplates(newFlowModalBody);
                   const templateList = getTemplateList(processTypesTemplates, SELECTORS.TEMPLATES_SECTION);
                   const templateItems = getTemplateItems(templateList);
                   expect(templateItems).toHaveLength(2);
                   expect(getTemplateItemTitle(templateItems[0])).toBe('myScreenFlowTemplateGetAccount');
                   expect(getTemplateItemTitle(templateItems[1])).toBe('screenFlowAddAccount');
               });
            });
        });
    });
