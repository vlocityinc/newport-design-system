...
const createComponentForTest = (node, mode, processType) => {
    const el = createElement(
        'builder_platform_interaction-record-delete-editor',
        { is: RecordDeleteEditor }
    );
...
describe('Record Delete Editor', () => {
    ...
    describe('name and dev name', () => {
        ...
        beforeAll(() => {
            uiFlow = translateFlowToUIModel(
                FLOWS_WITH_DELETE_RECORDS.USING_SOBJECT
            );
        ...
        it('do not change "dev name" if it already exists after the user modifies the "label"', () => {
            newLabel = 'new label';
            changeInputValue(
                getLabelDescriptionLabelElement(recordDeleteComponent),
                newLabel
            );
            return Promise.resolve().then(() => {
                expect(recordDeleteComponent.node.label.value).toBe(newLabel);
                expect(recordDeleteComponent.node.name.value).toBe(
                    DELETE_RECORDS_USING_SOBJECT_FLOW_ELEMENT
                );
            });
        });
        it('modify the "dev name"', () => {
            ...
            changeInputValue(
                getLabelDescriptionNameElement(recordDeleteComponent),
                newDevName
            );
            return Promise.resolve().then(() => {
    ...
    describe('Existing element', () => {
        describe('Working with sObject', () => {
            ...
            describe('Record Variable or Record Collection Variable picker', () => {
                ...
                it('Selected value is correctly displayed', () => {
                    expect(recordVariablePicker.value).toBe(
                        recordDeleteNode.inputReference.value
                    );
                });
        ...
        describe('Working with fields', () => {
            beforeAll(() => {
                uiFlow = translateFlowToUIModel(
                    FLOWS_WITH_DELETE_RECORDS.USING_FIELDS
                );
                store.dispatch(updateFlow(uiFlow));
            });
            afterAll(() => {
            ...
            beforeEach(() => {
                recordDeleteNode = getElementForPropertyEditor(
                    getElementByDevName(
                        DELETE_RECORDS_USING_FIELDS_FLOW_ELEMENT
                    )
                );
                recordDeleteComponent = createComponentForTest(
                    recordDeleteNode
            ...
            describe('Entity resource picker', () => {
                ...
                it('only deleteable entities available', () => {
                    ...
                    expect(comboboxItems).toContainEqual(
                        expect.objectContaining({ displayText: 'Account' })
                    );
                    expect(comboboxItems).toContainEqual(
                ...
                describe('Enter invalid value', () => {
                    ...
                    it('should display invalid entry error', () => {
                        return Promise.resolve().then(() => {
                            expect(
                                recordDeleteComponent.node.object.error
                            ).toBe(
            ...
            describe('Record Filter', () => {
                ...
                it('operators available for the first filter', () => {
                    const fieldToFerovExpressionBuilderComponents = getFieldToFerovExpressionBuilders(
                        recordFilter
                    );
    ...
    describe('sObject Or SObject Collection Picker', () => {
        ...
        beforeEach(() => {
            ...
            recordDeleteElement = createComponentForTest(
                recordDeleteNode,
