import { LABELS } from '../../apexPluginEditor/apexPluginEditorLabels';
...
describe('apexPlugin ParameterList warnings', () => {
    describe('getParameterItemWarning', () => {
        describe('parameter is not available in apex plugin', () => {
            let warning;
            beforeAll(() => {
            ...
            it('should have "will cause errors" badge', () => {
                expect(warning.warningBadge).toEqual('FlowBuilderApexPluginEditor.badgeWillCauseErrors');
            });
            it('should have a warning message', () => {
                expect(warning.warningMessage).toEqual('FlowBuilderApexPluginEditor.warningNotAvailable');
            });
        });
        describe('Duplicates', () => {
            ...
            it('should have a badge and warning message if there is another warning for the same parameter', () => {
                const warning = getParameterItemWarning(
                    [MERGE_WARNING_TYPE.DUPLICATE, MERGE_WARNING_TYPE.NOT_AVAILABLE],
                    LABELS
                );
                expect(warning).toEqual({
                    grayPill: false,
                    shouldBeDeleted: true,
                    hideIcon: true,
                    warningBadge: 'FlowBuilderApexPluginEditor.badgeWillCauseErrors',
                    warningMessage: 'FlowBuilderApexPluginEditor.warningNotAvailable'
                });
            });
        });
    });
