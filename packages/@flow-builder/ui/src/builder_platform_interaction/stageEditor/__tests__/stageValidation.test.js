...
describe('Default Validations', () => {
    describe('Stage Order property', () => {
        it('should return an error if null.', () => {
            expect(stageValidation.validateProperty('stageOrder', null)).toBe(
                LABELS.shouldBeAPositiveIntegerOrZero
            );
        });
        it('should return an error if NaN or exponential value is entered.', () => {
            expect(stageValidation.validateProperty('stageOrder', 'e4')).toBe(
                LABELS.shouldBeAPositiveIntegerOrZero
            );
        });
        it('should return an error if negative value is entered.', () => {
            expect(stageValidation.validateProperty('stageOrder', '-4')).toBe(
                LABELS.shouldBeAPositiveIntegerOrZero
            );
        });
        ...
        it('should return an error if empty string value is entered.', () => {
            expect(stageValidation.validateProperty('stageOrder', '')).toBe(
                LABELS.cannotBeBlank
            );
        });
        it('should return an error if value entered is more than max limit.', () => {
            expect(
                stageValidation.validateProperty('stageOrder', '1000000000000')
