...
describe('UndoRedo Library Function', () => {
    ...
    describe('UndoRedo function - Switch case - Default', () => {
        it('Expect reducer to be called in case the action type is not undo or redo', () => {
            ...
            expect(mockReducerFn).toHaveBeenCalledWith(
                initialStoreState,
                mockInitAction
            );
        });
        it('Expect the return state from undo redo to be same as initialStateStore', () => {
            const result = undoRedoFnWithMockReducer(
                initialStoreState,
                mockInitAction
            );
        ...
        it('Expect No state is added to past until all the consecutive actions are the same groupedAction', () => {
            const stateAfterInit = undoRedoFnWithMockReducer(
                initialStoreState,
                mockInitAction
            ); // Init action does not create a past state.
            const stateAfterOneGroupedAction = undoRedoFnWithMockReducer(
                stateAfterInit,
                mockGroupedAction
            );
            undoRedoFnWithMockReducer(
    ...
    describe('UndoRedo function - Switch Case - Undo', () => {
        ...
        it('Brings back previous State after undo is pressed', () => {
            ...
            const stateAfterFirstAction = undoRedoFnWithMockReducer(
                stateAfterInit,
                mockTestAction
            );
            const stateAfterSecondAction = undoRedoFnWithMockReducer(
                storeStateWithOneMockElement,
                mockTestAction
            );
            const undoActionResult = undoRedoFnWithMockReducer(
                stateAfterSecondAction,
                mockUndoAction
            );
    ...
    describe('UndoRedo function - Switch case - Redo', () => {
        ...
        it('to return the first state stored in future array', () => {
            ...
            const stateAfterUndoAction = undoRedoFnWithMockReducer(
                stateAfterSecondAction,
                mockUndoAction
            );
            const redoActionResult = undoRedoFnWithMockReducer(
                stateAfterUndoAction,
                mockRedoAction
            );
