...
/**
 * Function to iteratively increment autogenerated label until a unique label is found
 *
 * @param labels list of possibly conflicting labels
 * @param labelFunction function which returns label
 * @returns unique label string
 */
function dedupeLabel(labels: (string | null | undefined)[], labelFunction: (counter: number) => string): string {
    const usedLabels = new Set(labels);
    let counter = 0;
    let label = labelFunction(counter);
    while (usedLabels.has(label)) {
        counter++;
        label = labelFunction(counter);
    }
    // Check for previously highest number used and generate the next number in sequence
    let limit = counter + 10;
    while (counter < limit) {
        counter++;
        const nextLabel = labelFunction(counter);
...
/**
 * Generates a default label for new elements being edited in a panel property editor
 *
 * @param elementType element type string
 * @param parent parent element guid
 * @returns unique label string
 */
export function generateDefaultLabel(elementType: string, parent?: string): string {
    ...
    if (parentElement?.childReferences) {
        ...
        return dedupeLabel(existingLabels, (counter) => {
            return format(
