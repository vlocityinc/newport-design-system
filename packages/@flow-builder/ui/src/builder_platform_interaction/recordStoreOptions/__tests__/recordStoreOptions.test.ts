...
describe('record-store-options default', () => {
    ...
    test('Check Radio Button default is firstRecord', () => {
        expect(getNumberRecordsToStoreRadioGroup(recordStoreOptionComponent).value).toBe(
            NUMBER_RECORDS_TO_STORE.FIRST_RECORD
        );
    });
    test('Check Radio Button default is SObject Variable', () => {
        expect(getWayToStoreFieldsRadioGroup(recordStoreOptionComponent).value).toBe(
    ...
    test('Event RecordStoreOptionChangedEvent is dispatched after selected radio button changed', async () => {
        ...
        getNumberRecordsToStoreRadioGroup(recordStoreOptionComponent).dispatchEvent(
            getChangeEvent(NUMBER_RECORDS_TO_STORE.ALL_RECORDS)
        );
        await Promise.resolve();
        expect(eventCallback).toHaveBeenCalled();
        expect(eventCallback.mock.calls[0][0].detail).toMatchObject(
            getRecordStoreOptionChangedEventDetail(false, false, WAY_TO_STORE_FIELDS.SOBJECT_VARIABLE)
        );
    });
    test('checkbox "Assign Null if no record found" changed', async () => {
        ...
        expect(eventCallback).toHaveBeenCalled();
        expect(eventCallback.mock.calls[0][0].detail).toMatchObject(
            getRecordStoreOptionChangedEventDetail(true, true, WAY_TO_STORE_FIELDS.SOBJECT_VARIABLE)
        );
    });
    test('Event RecordStoreOptionChangedEvent is dispatched after selected way to store fields radio button changed to SObject Variable', async () => {
        ...
        expect(eventCallback).toHaveBeenCalled();
        expect(eventCallback.mock.calls[0][0].detail).toMatchObject(
            getRecordStoreOptionChangedEventDetail(false, true, WAY_TO_STORE_FIELDS.SOBJECT_VARIABLE)
        );
    });
    test('Event RecordStoreOptionChangedEvent is dispatched after selected way to store fields radio button changed to Separate Variables', async () => {
        ...
        expect(eventCallback).toHaveBeenCalled();
        expect(eventCallback.mock.calls[0][0].detail).toMatchObject(
            getRecordStoreOptionChangedEventDetail(false, true, WAY_TO_STORE_FIELDS.SEPARATE_VARIABLES)
        );
    });
});
