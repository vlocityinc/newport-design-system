 ...
 * @private
 */
const _checkIfMarqueeOverlapsCanvasElement = (canvasElementStartPoint, canvasElementEndPoint, marqueeBoxStartPoint, marqueeBoxEndPoint) => {
    ...
    return !(canvasElementEndPoint[0] < marqueeBoxMinX ||   // Canvas element is on the left of the box
             ...
             canvasElementStartPoint[1] > marqueeBoxMaxY    // Canvas element is on the bottom of the box
            );
};
...
const _getMarqueePointsRelativeToViewportCenter = (marqueeStartPoint, marqueeEndPoint, viewportCenterPoint) => {
    const relativeMarqueeStartPoint = [marqueeStartPoint[0] - viewportCenterPoint[0], marqueeStartPoint[1] - viewportCenterPoint[1]];
    const relativeMarqueeEndPoint = [marqueeEndPoint[0] - viewportCenterPoint[0], marqueeEndPoint[1] - viewportCenterPoint[1]];
 ...
 * @private
 */
const _getElementLocationRelativeToViewportCenter = (locationX, locationY, currentScale, scaledOffsetsOnMarqueeStart, viewportCenterPoint) => {
    ...
    const relativeElementLocationX = ((locationX - viewportCenterPoint[0]) * currentScale) + scaledOffsetsOnMarqueeStart[0];
    const relativeElementLocationY = ((locationY - viewportCenterPoint[1]) * currentScale) + scaledOffsetsOnMarqueeStart[1];

    // Calculating the start and end point of the canvas element using the element dimensions and current scale
    const relativeElementStartPoint = [relativeElementLocationX, relativeElementLocationY];
    const relativeElementEndPoint = [relativeElementLocationX + (NODE_LENGTH * currentScale), relativeElementLocationY + (NODE_LENGTH * currentScale)];
 ...
 * @private
 */
const _getCanvasElementGuidsToSelectAndDeselect = (canvasElements, currentScale, { scaledOffsetsOnMarqueeStart, marqueeStartPoint, marqueeEndPoint }, viewportCenterPoint) => {
    ...
    const { relativeMarqueeStartPoint, relativeMarqueeEndPoint } =
        _getMarqueePointsRelativeToViewportCenter(marqueeStartPoint, marqueeEndPoint, viewportCenterPoint);
    ...
    canvasElements.forEach(({ locationX, locationY, config, guid, elementType }) => {
        ...
        const { relativeElementStartPoint, relativeElementEndPoint } =
            _getElementLocationRelativeToViewportCenter(locationX, locationY, currentScale, scaledOffsetsOnMarqueeStart, viewportCenterPoint);

        // Using the relative locations to check if a canvas element is overlapping with the marquee selection box or not
        const isMarqueeOverlappingCanvasElement =
            _checkIfMarqueeOverlapsCanvasElement(relativeElementStartPoint, relativeElementEndPoint, relativeMarqueeStartPoint, relativeMarqueeEndPoint);
        ...
        if (!wasCanvasElementOriginallySelected && isMarqueeOverlappingCanvasElement && elementType !== 'START_ELEMENT') {
    ...
    return { canvasElementGuidsToSelect, canvasElementGuidsToDeselect, allSelectedCanvasElementGuids };
};
...
export const checkMarqueeSelection = (canvasElements, connectors, currentScale, marqueeConfig, viewportCenterPoint) => {
    ...
    const { connectorGuidsToSelect, connectorGuidsToDeselect } =
        _getConnectorGuidsToSelectAndDeselect(connectors, allSelectedCanvasElementGuids);
