 ...
 * @private
 */
const _checkForUndefined = (currentScale, viewportWidth, viewportHeight, centerOffsetX, centerOffsetY, canvasElements) => {
...
export const canZoom = (event, isCanvasMouseDown, isMarqueeInProgress) => {
    return event && (event.metaKey || event.ctrlKey)
        ...
        && !isCanvasMouseDown && !isMarqueeInProgress;
};
 ...
 */
export const getScaleAndOffsetValuesOnZoom = (action, currentScale, { viewportDimensions, centerOffsets }, canvasElements) => {
    ...
    if (action === ZOOM_ACTION.ZOOM_OUT && currentScale !== undefined) {
    ...
    } else if (action === ZOOM_ACTION.ZOOM_TO_FIT && _checkForUndefined(currentScale, viewportDimensions.viewportWidth, viewportDimensions.viewportHeight, centerOffsets[0], centerOffsets[1], canvasElements)) {
        ...
        newScaledOffsetLeft = (viewportDimensions.viewportCenterPoint[0] - (flowBounds.minX + (flowBounds.flowWidth / 2))) * newScale;
        newScaledOffsetTop = (viewportDimensions.viewportCenterPoint[1] - (flowBounds.minY + (flowBounds.flowHeight / 2))) * newScale;
 ...
 */
export const getDistanceBetweenViewportCenterAndElement = (viewportCenterPoint, elementLocationX, elementLocationY, currentScale) => {
