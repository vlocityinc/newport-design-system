...
To update autolaunchedFlowScheduledJourney.json (and other):

1) create a new flow named "autolaunchedFlowScheduledJourney" and save it (you can move the start element to have the save button enabled)
2) add a breakpoint in TranslatorLib.translateFlowToUIModel
3) refresh the page (F5)
4) in the console :
flow = [paste from the contents of autolaunchedFlowScheduledJourney.json]
...
10) update metadata (only !) in autolaunchedFlowScheduledJourney.json 
