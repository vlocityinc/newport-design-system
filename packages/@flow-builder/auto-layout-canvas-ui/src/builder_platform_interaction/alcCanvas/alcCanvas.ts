...
export default class AlcCanvas extends withKeyboardInteractions(LightningElement) {
    ...
    updateFlowRenderContext(flowRenderContext: Partial<FlowRenderContext> = {}) {
        ...
        if (isFirstRender) {
            ...
            const nodes: { elementType: string; elementSubtype: string; actionName: string }[] = Object.values(
                this.flowModel
            );
            let count = 0;
            nodes.forEach((node) => {
    ...
    handlePasteOnCanvas = (event: PasteOnCanvasEvent) => {
        ...
        if (options.isCutPaste) {
            event.stopPropagation();
            const guid = this.canvasContext.cutInfo.guids[0];
            ...
            this.moveFocusToIncomingConnector(guid);
