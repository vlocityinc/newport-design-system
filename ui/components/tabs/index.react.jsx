...
class TabContent extends React.Component {
  render() {
    ...
    const classNameComputed = classNames(
      className,
      classNames(`nds-tabs_${flavor}__content`, {
        'nds-show': current,
        'nds-hide': !current
      })
    );
...
class TabItem extends React.Component {
  renderCustom(tabIndex) {
    return React.cloneElement(this.props.content, {
      tabIndex: tabIndex,
      className: `nds-tabs_${this.props.flavor}__link`,
  ...
  render() {
    ...
    const classNameComputed = classNames(
      className,
      classNames(`nds-tabs_${flavor}__item`, {
        'nds-is-active': current
      })
    );
...
class TabItemOverflow extends React.Component {
  render() {
    ...
    const classNameComputed = classNames(
      className,
      classNames('nds-tabs__item_overflow', {
        'nds-is-active': current
      })
    );
    const tabIndex = current ? 0 : -1;
...
class Tabs extends React.Component {
  ...
  render() {
    ...
    const composedClassName = classNames(className, `nds-tabs_${flavor}`);
    return (
      <div {...rest} className={composedClassName}>
        <ul className={`nds-tabs_${flavor}__nav`} role="tablist">
          {this.tabs()}
        </ul>
